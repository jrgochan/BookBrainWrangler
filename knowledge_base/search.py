...
from knowledge_base.config import (DEFAULT_SEARCH_LIMIT,
...
def get_recent_documents(kb: KnowledgeBase,
                         limit: int = 5,
    ...
    try:
        ...
        if max_age_days > 0:
            ...
            documents = [
                ...
                if 'date_added' not in doc or datetime.fromisoformat(
                    doc.get('date_added', '2020-01-01')) > cutoff_date
            ]

        # Sort by date added (most recent first)
        documents.sort(key=lambda x: datetime.fromisoformat(
            x.get('date_added', '2020-01-01')),
                       reverse=True)
        ...
        for doc in documents:
            ...
            for chunk in doc_chunks:
                ...
                chunk['metadata']['title'] = doc.get('metadata', {}).get(
                    'title', 'Untitled')
                chunk['metadata']['author'] = doc.get('metadata', {}).get(
                    'author', 'Unknown')
